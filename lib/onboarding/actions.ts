...
export async function completeOnboarding(
  ...
  if (!termsAccepted) return { error: "Please accept the Terms and Conditions." };
  ...
  const { error } = await supabase
    .from("profiles")
    .update({ course_id: course.id, major_id: majorId, terms_accepted_at: new Date().toISOString() })
    .eq("id", user.id);
